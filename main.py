...
model = YOLO("C:/best.pt")  # best.pt 경로 확인 후 수정
...
try:
    while True:
        ...
        # 2️⃣ 이미지 색감 보정 (드론 카메라 문제 해결)
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # BGR → RGB 변환
        frame = cv2.convertScaleAbs(frame, alpha=1.2, beta=10)  # 밝기 & 대비 조정

        # 3️⃣ YOLO 모델로 얼굴 감지 및 분류
        results = model(frame)
        ...
        for result in results:
            for box in result.boxes:
                ...
                if confidence > 0.5:  # 신뢰도 50% 이상인 경우만 표시
                    ...
                    color = (0, 255, 0) if label == "songhee" else (255, 0, 0)  # 색상 구분 (송희: 초록, 희지: 파랑)
